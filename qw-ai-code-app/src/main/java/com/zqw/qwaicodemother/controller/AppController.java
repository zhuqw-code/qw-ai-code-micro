...
import com.zqw.qwaicodemother.model.dto.app.*;
...
import com.zqw.qwaicodemother.service.ProjectDownloadService;
...
public class AppController {
    ...
    @RateLimit(limitType = RateLimitType.USER, rate = 2, rateInterval = 60, message = "AI请求过于频繁，请稍后再试~")
    ...
    @Cacheable(
            value = "good_app_page",  // value 是为了给「一整组 key」（即一个缓存分区）设置参数
            key = "T(com.zqw.qwaicodemother.utils.CacheKeyUtils).generateKey(#appQueryRequest)",  // 设置key的生成策略
            condition = "#appQueryRequest.pageNum <= 10" // 添加缓存生效的条件
    )
